...
\begin{abstract}
Othello, also named Reversi, is a strategy board game between two players. As an artificial intelligence game application, what a player is facing is an intelligence agent instead of another human. This report describes in detail about the AI methodology, such as the search engine, $\alpha$-$\beta$ pruning, heuristics applied, and the application architecture. Some other detail about the program like how Horizon Effect is avoided, the number of steps the AI looks ahead, and the precoded-database in use. In the end, instructions on how to deploy and run the game are given.
\end{abstract}
